...
**결정**: **승인** (오너, 2026-07-06) → 수정 커밋 `c878b06`. 플레이테스트 검증 대기.

## #2026-07-06-6 — [오너 결정 필요] BUG-4 몬스터 넉백 클램프

**배경**: 몬스터 넉백 2경로(`EnemyCombat.ApplySelfKnockback`, `EnemyStatusController.ApplyKnockback`)가 `rb.MovePosition` 단발 순간이동으로 벽/충돌 검사가 없음. 문 근처에서 몬스터가 통로 밖으로 순간이동함.

**제안 (코드-온리)**: 두 경로에 `rb.Cast` 기반 이동거리 클램프 추가 (Player 넉백의 기존 안전 패턴 준용). 옵션: 현재 방 사각형 경계 클램프까지 포함하면 BUG-3의 "방 밖 이탈" 측면도 함께 완화.

**검증 필요**: 몬스터 넉백 체감(막히는 느낌), 벽 모서리 끼임 여부.

**결정**: (대기)

## #2026-07-06-7 — [오너 결정 필요] BUG-5 투사체 판정 기준

**배경**: 보스 투사체는 태그 게이트 때문에 정밀 히트박스(0.6배)가 아닌 **본체 캡슐 전체**에 명중 판정. 근접 판정과 이중 기준.

**선택지**: (a) 투사체도 CombatTargetHitbox(0.6배) 기준으로 통일(판정 좁아짐 = 난이도 하향 체감), (b) 현행 유지 + 프리팹별 콜라이더만 에디터 실측 조정, (c) a+b.
