...
public class RoomManager : MonoBehaviour
{
    ...
    private void RefreshRoomState()
    {
        ...
        if (matchData != null)
        {
            ...
            {
                if (loadedRooms.TryGetValue(matchData.roomID, out var existingObj) && existingObj != null)
    ...
    public void RequestMove(Vector2 direction, RoomData nextRoom, float distanceOverride = 0f)
    {
        ...
        {
            if (loadedRooms.TryGetValue(nextRoom.roomID, out var existingRoom) && existingRoom != null)
            {
                existingRoom.SetActive(true);

                if (!runtimeRoomPositions.ContainsKey(nextRoom.roomID))
                    runtimeRoomPositions[nextRoom.roomID] = existingRoom.transform.position;

                DLog($"RequestMove: nextRoom already loaded [{nextRoom.roomID}] at {existingRoom.transform.position}");
            }
        }
    ...
    private void ResolvePlayerReference(bool activateIfFoundViaTag)
    {
        ...
        playerComponent = foundPlayer.GetComponent<Player>();
    ...
    private void ClampPlayerToWalkablePosition()
    {
        if (player == null)
        {
            return;
        ...
        playerComponent.SnapToWalkablePosition();
    }
